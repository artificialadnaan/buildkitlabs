...
// ── Grid config — asymmetric, varied card types ────────────────────
// textOnly: cards where screenshot is a login page — lead with metrics instead
const gridConfig: Record<string, {
  ...
  'skyguard-hq':      { className: 'col-span-12 md:col-span-5', imgH: '0', textOnly: true },
  'buildkit-crm':     { className: 'col-span-12 md:col-span-5', imgH: '0', textOnly: true },
  'ticket-hub':       { className: 'col-span-12 sm:col-span-6 md:col-span-5', imgH: '160px', wide: true },
  'virasat-jewels':   { className: 'col-span-12 sm:col-span-6 md:col-span-4', imgH: '180px' },
  'trock-website':    { className: 'col-span-12 sm:col-span-6 md:col-span-3', imgH: '140px' },
  'skyguard-website': { className: 'col-span-12 sm:col-span-6 md:col-span-3', imgH: '160px' },
  fencetastic:        { className: 'col-span-12 sm:col-span-6 md:col-span-5', imgH: '180px', wide: true },
    ...
    >
      ...
      <div className="p-5 pl-6 flex flex-col h-full">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] tracking-[0.2em] text-stone-500 uppercase">{project.subtitle}</span>
          {project.nda && (
        ...
        <h3 className="text-xl font-bold text-stone-100 tracking-tight">{project.name}</h3>
        <p className="text-[13px] text-stone-500 mt-1.5 line-clamp-2 leading-relaxed">{project.description}</p>

        {/* Lead with the big metric */}
        <div className="mt-4">
          <span className="text-3xl font-bold text-stone-100">{project.metrics[0].value}</span>
          <span className="text-xs text-stone-500 uppercase tracking-wider ml-2">{project.metrics[0].label}</span>
        </div>
...
function BentoCard({ project, index, onClick }: { project: Project; index: number; onClick: () => void }) {
  ...
  const isTextOnly = config.textOnly
  ...
  // Text-only cards (login page screenshots)
  if (isTextOnly) {
    ...
    return (
      ...
      >
        <div className="relative flex-shrink-0 h-[280px] overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-transparent from-40% to-dark-950/95" />
  ...
  return (
    ...
    >
      <div className="relative flex-shrink-0 overflow-hidden" style={{ height: imgHeight }}>
        <img src={project.screenshot} alt="" className="w-full h-full object-cover object-top" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent from-50% to-dark-950/80" />
...
export default function ShowcaseGrid() {
  ...
  return (
    <div className="min-h-screen bg-dark-950">
      {/* Warm amber radial wash — subtle, matches homepage */}
      <div
      ...
      />
      {/* Blueprint grid — matches homepage */}
