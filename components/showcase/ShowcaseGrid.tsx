...
const gridConfig: Record<string, { className: string; minH: string; isHero?: boolean }> = {
  synchub:            { className: 'col-span-12 md:col-span-7 md:row-span-2', minH: '500px', isHero: true },
  'skyguard-hq':      { className: 'col-span-12 md:col-span-5', minH: '240px' },
  'buildkit-crm':     { className: 'col-span-12 md:col-span-5', minH: '240px' },
  'ticket-hub':       { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '280px' },
  'virasat-jewels':   { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '280px' },
  'trock-website':    { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '280px' },
  'skyguard-website': { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '240px' },
  fencetastic:        { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '240px' },
  'booth-plug':       { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '240px' },
  'buildkit-labs':    { className: 'col-span-12 md:col-span-12', minH: '200px' },
}
...
const accentGlows: Record<string, string> = {
  ...
  'buildkit-crm':     '#EAB308',
  'ticket-hub':       '#F97316',
  'virasat-jewels':   '#8B5CF6',
  'trock-website':    '#EF4444',
  'skyguard-website': '#06B6D4',
  fencetastic:        '#9CA3AF',
  'booth-plug':       '#DAA520',
...
function BentoCard({
  ...
  const config = gridConfig[project.id] ?? { className: 'col-span-12 sm:col-span-6 md:col-span-4', minH: '260px' }
  ...
  if (isHero) {
    return (
      ...
      >
        {/* Accent glow */}
        <div
          className="absolute top-0 right-0 w-[300px] h-[300px] opacity-[0.06] group-hover:opacity-[0.12] transition-opacity duration-[400ms] pointer-events-none"
        ...
        {/* NDA badge */}
        {project.nda && (
          <div className="absolute top-4 right-4 z-10 flex items-center gap-1.5 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10">
            <svg className="w-3 h-3 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span className="text-xs text-white/70">NDA</span>
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
          ...
          <p className="text-sm text-white/50 mt-1 line-clamp-1 max-w-lg">{project.description}</p>
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            {project.tech.slice(0, 5).map((t) => (
              <span key={t} className="bg-white/10 backdrop-blur-sm text-white/70 text-[11px] px-2.5 py-1 rounded-md">{t}</span>
            ))}
            {project.tech.length > 5 && (
              <span className="bg-white/10 backdrop-blur-sm text-white/40 text-[11px] px-2.5 py-1 rounded-md">+{project.tech.length - 5}</span>
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      {/* Accent glow */}
      <div
        className="absolute top-0 right-0 w-[200px] h-[200px] opacity-[0.06] group-hover:opacity-[0.12] transition-opacity duration-[400ms] pointer-events-none"
        style={{ background: `radial-gradient(circle, ${glow} 0%, transparent 70%)` }}
      />
      ...
      <div className="relative flex-shrink-0" style={{ height: '55%' }}>
        ...
        {project.nda && (
          <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-white/10 backdrop-blur-md px-2.5 py-1 rounded-lg border border-white/10">
            <svg className="w-3 h-3 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span className="text-[10px] text-white/70">NDA</span>
          </div>
        )}
      </div>
        ...
        <div className="flex items-center gap-4 mt-2 flex-wrap">
          {project.metrics.slice(0, 2).map((m, i) => (
            <div key={i}>
              ...
              <span className="text-[10px] text-white/40 uppercase tracking-wider ml-1">{m.label}</span>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <div className="p-8">
          ...
          <div className="flex items-center gap-3 mt-8">
            ...
            <button onClick={onClose} className="px-6 py-3 text-sm text-white/40 hover:text-white transition-colors">
              &larr; Back
            </button>
          </div>
        </div>
